	...
	for j := 0; j < len(cfg.logs); j++ {
		...
		if ok && (e != logEntry) {
			log.Fatalf("server %d is trying to commit logEntry: %+v at index: %d "+
				"but server: %d has already committed: %+v for the same index",
	...
	for time.Since(t0).Seconds() < 10 {
		...
		for si := 0; si < cfg.n; si++ {
			...
			if rf != nil {
				...
				// log.Printf("Config - Node %d, command %d", rf.me, cmd)
				if ok {
					index = index1
					break
				}
				// log.Printf("Server %d is not the leader because it said so", rf.me)
			}
		}
		...
		// log.Printf("Was not able to commit in time, retrying")
	}
	// log.Fatalf("one(%v) failed to reach agreement within 10 seconds - very slows", cmd)
	return -1
}
